...
void Client::Receive() {
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        while (received_size < size_of_file) {
            ...
            percentage = (((float)Client::statistics.received_size * 100.f) / (float)Client::statistics.total_size);
            wxLogStatus(wxT("Receiving... : %d / %d ( %d% )"), Client::statistics.current_count, Client::statistics.total_count, percentage);
        }
        if (percentage == 100 && (Client::statistics.current_count == Client::statistics.total_count)) {
            wxLogMessage(wxT("Completed"));
            wxLogStatus(wxT("Received successfully"));
        }
